...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a Gaussai address"));
    ...
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a Gaussai address").arg(strCoinJoinName));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and gaussai: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gaussai addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gaussai addresses"));
    m_load_psbt_action = new QAction(tr("Load PSBT..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Gaussai Transaction"));
    ...
    openAction->setStatusTip(tr("Open a gaussai: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gaussai command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Gaussai network", "", count));
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the gaussai logo
