...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GSS);
    ...
    if (!settings.contains("nCoinJoinAmount")) {
        ...
        if (!settings.contains("nAnonymizeGaussaiAmount"))
            settings.setValue("nCoinJoinAmount", DEFAULT_COINJOIN_AMOUNT);
        else
            settings.setValue("nCoinJoinAmount", settings.value("nAnonymizeGaussaiAmount").toInt());
    }
