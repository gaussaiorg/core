...
const char * const BITCOIN_CONF_FILENAME = "gaussai.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //gaussai-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Gaussai
    // macOS: ~/Library/Application Support/Gaussai
    // Unix-like: ~/.gaussai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gaussai";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Gaussai";
#else
    // Unix-like
    return pathRet / ".gaussai";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty gaussai.conf if it does not exist
