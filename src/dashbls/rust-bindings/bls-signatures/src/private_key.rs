...
impl PrivateKey {
    ...
    #[cfg(feature = "gaussai_helpers")]
    pub fn generate_gaussai(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "gaussai_helpers")]
    pub fn generate_gaussai_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_gaussai(rng))
            .collect()
    }
