...
TMPDIR_PREFIX = "gaussai_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave gaussaids and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop gaussaids after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use gaussai-cli instead of RPC for all commands")
        parser.add_argument("--gaussaid-arg", dest="gaussaid_extra_args", default=[], action="append",
                            help="Pass extra args to all gaussaid instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "gaussaid" + config["environment"]["EXEEXT"]
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "gaussai-cli" + config["environment"]["EXEEXT"]
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.gaussaid_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: gaussaids were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'gaussaid', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'gaussai-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "gaussai.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as gaussaid's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in gaussai.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if gaussaid has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("gaussaid has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if gaussai-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("gaussai-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if gaussai-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("gaussai-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether gaussai-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether gaussai-wallet was compiled."""
...
class GaussaiTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_gaussai_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, evo_count=0):
        ...
        self.set_gaussai_dip8_activation(200)
        ...
        # This is nRequestTimeout in gaussai-q-recovery thread
    ...
    def set_gaussai_dip8_activation(self, activate_after_block):
    ...
    def set_gaussai_llmq_test_params(self, llmq_size, llmq_threshold):
