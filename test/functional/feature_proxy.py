...
"""Test gaussaid with different proxy configuration.

Test plan:
- Start gaussaid's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on gaussaid side:
...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: gaussaid's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
