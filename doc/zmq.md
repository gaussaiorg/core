...
The Gaussai Core daemon can be configured to act as a trusted "border
router", implementing the gaussai wire protocol and relay, making
...
The ZeroMQ feature in Gaussai Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building gaussaid:
...
For instance:
    ...
    $ gaussaid -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/gaussaid.tx.raw \
...
These options can also be provided in gaussai.conf.
...
From the perspective of gaussaid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into gaussaid directly. Furthermore, no information is
...
using. Gaussaid appends an up-counting sequence number to each
