...
- [Development guidelines](#development-guidelines)
    - [General Gaussai Core](#general-gaussai-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Gaussai Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Gaussai Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/gaussaiorg/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_gaussai
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_gaussai --log_level=test_suite
$ valgrind -v --leak-check=full src/gaussaid -printtoconsole
...
# A coverage report will now be accessible at `./test_gaussai.coverage/index.html`.
```
...
To profile a running gaussaid process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep gaussaid` -- sleep 60
```
...
Gaussai Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`gaussaid`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 15906.
...
However, in open source software such as Gaussai Core, where everyone uses
...
pay attention to for reviewers of Gaussai Core code.

General Gaussai Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Gaussai Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Gaussai Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `gaussai-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `gaussai-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Gaussai Core, the wallet was always
...
- Use *invalid* Gaussai addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
